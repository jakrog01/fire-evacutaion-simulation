...
class MapVisualizer:
    ...
    def _plot_nav(self, ax: Axes) -> None:
    ...
    def _plot_blocking(self, ax: Axes) -> None:
        im = ax.imshow(self.f_block, cmap="magma", vmin=0, vmax=11, origin="upper")
        ax.set_title("Blocking Fuel")
        plt.colorbar(im, ax=ax)

    def _plot_ceiling(self, ax: Axes) -> None:
        im = ax.imshow(self.f_ceil, cmap="magma", vmin=0, vmax=11, origin="upper")
        ax.set_title("Ceiling Fuel")
        plt.colorbar(im, ax=ax)

    def _plot_patron_potential(self, ax: Axes) -> None:
    ...
    def _plot_staff_potential(self, ax: Axes) -> None:
    ...
    def _plot_fire_on_architecture(self, ax: Axes) -> None:
        rgb_image = np.zeros((*self.nav.shape, 3), dtype=np.uint8)
