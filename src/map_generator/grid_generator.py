...
from .map_loader import MapLoaderConfig, load_config
...
class GridGenerator:
    ...
    @property
    def config(self) -> MapLoaderConfig:
        return self._config
    ...
    def generate_fire_matrices(self) -> FireSimulationMaps:
        ...
        self.fire_blocking, self.fire_nonblocking = fire_gen.generate_all()
    ...
    def visualize_final(self) -> None:
        ...
        if (
            self.nav is None
            or self.f_block is None
            or self.f_ceil is None
            or self.pot_patron is None
            or self.pot_staff is None
        ):
            raise RuntimeError(
                "Maps and potentials must be generated before visualization"
            )
